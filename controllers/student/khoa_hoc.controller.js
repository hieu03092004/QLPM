...
//                 "thoi_gian_ket_thuc": "09:50",
//                 "ngay_bat_dau": "2023-10-24",
//                 "ngay_ket_thuc": "2023-12-12"
//             }
//         }
//     ]
// }
module.exports.chiTietNhomThucHanh = async (req, res) => {
  try {
    ...
    // 1. Lấy thông tin nhóm thực hành để lấy mã phòng
    const { data: nhomThucHanh, error: nhomThucHanhError } = await supabase
      .from('NhomThucHanh')
      .select('phong_thuc_hanh')
      .eq('ma_nhom_thuc_hanh', ma_nhom_thuc_hanh)
      .single();
    if (nhomThucHanhError || !nhomThucHanh) {
      return res.status(404).json({
        statusCode: 404,
        success: false,
        error: 'Không tìm thấy thông tin nhóm thực hành'
      });
    }

    const ma_phong = nhomThucHanh.phong_thuc_hanh;

    // 2. Lấy tất cả phiếu đăng ký của nhóm này
    const { data: phieuDangKy, error: phieuDangKyError } = await supabase
      .from('PhieuDangKy')
      .select('ma_phieu')
      .eq('ma_nhom_thuc_hanh', ma_nhom_thuc_hanh);
    ...
    // 3. Lấy tất cả chi tiết đăng ký theo các phiếu này
    const { data: chiTietDangKy, error: chiTietDangKyError } = await supabase
      .from('ChiTietDangKy')
      .select('ma_tuan, ma_ca_thu, ngay_hoc')
      .in('ma_phieu', maPhieuArr);
    ...
    // 4. Lấy danh sách ma_ca_thu duy nhất
    const uniqueMaCaThu = [
      ...new Set(chiTietDangKy.map(item => item.ma_ca_thu))
    ];

    // 5. Lấy thông tin ca/thứ từ bảng CaHoc_ThuTrongTuan
    const { data: listCaThu, error: caThuError } = await supabase
      .from('CaHoc_ThuTrongTuan')
      .select('ma_ca_thu, ma_ca, ma_thu')
      .in('ma_ca_thu', uniqueMaCaThu);
    ...
    // 6. Gom tuần, ngày bắt đầu/kết thúc, lấy lichHoc đầu tiên
    const Tuan = [];
    const ngayArr = [];
    let lichHoc = null;
    ...
    chiTietDangKy.forEach(item => {
      ...
      if (!lichHoc && caThuMap[item.ma_ca_thu]) {
        lichHoc = {
          thu: caThuMap[item.ma_ca_thu].thu,
          ca: caThuMap[item.ma_ca_thu].ca,
          thoi_gian_bat_dau: CaHoc[0].gio_bat_dau.substr(0, 5),
          thoi_gian_ket_thuc: CaHoc[0].gio_ket_thuc.substr(0, 5),
          ngay_bat_dau: item.ngay_hoc,
          ngay_ket_thuc: item.ngay_hoc
        }
      }
    ...
    return res.status(200).json({
      ...
      data: [
        {
          phong: ma_phong, // Lấy từ bảng NhomThucHanh
          Tuan,
          lichHoc
        }
      ]
